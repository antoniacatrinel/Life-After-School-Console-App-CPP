...
void Tests::test_service()
{
    ...
    RepositoryUser* eventsList = new CSVEventsList();

    Service srv{ fileRepo, eventsList };

    Event e{"festival", "cluj-napoca", "13/09/2022", "18:00", 15000, "https"};
    srv.addEventService("festival", "cluj-napoca", "13/09/2022", "18:00", 15000, "https");
    assert(srv.getRepo().getEvents().size() == 1);
    assert(srv.getRepo().getEvents()[0] == e);
    assert(srv.findByTitleDate("festival", "13/09/2022") == e);
    try {
        srv.findByTitleDate("aaa", "11/10/2022");
    }
    catch (RepositoryException& rerr)
    {
        assert(rerr.what() == "Repository error: nonexistent event!");
    ...
    Event e1{"marathon", "timisoara", "16/10/2023", "10:00", 5000, "url"};
    Event e2{};
    srv.deleteEventService("festival", "13/09/2022");
    assert(srv.getRepo().getRepoSize() == 0);

    srv.addEventService("festival", "cluj-napoca", "13/09/2022", "18:00", 15000, "https");
    assert(srv.getRepo().getRepoSize() == 1);
    srv.updateEventService(e, e1);
    srv.updateEventService(e2, e1);
    assert(srv.getRepo().getRepoSize() == 1);

    assert(srv.getMonth("13/12/2020") == 12);
    assert(srv.getMonth("13/02/2020") == 2);

    assert(srv.compareDates(e, e1) == true);
    assert(srv.compareDates(e1, e) == false);
    assert(srv.compareDates(e, e) == true);

    assert(srv.getRepo().getEvents()[0].getDate() == "16/10/2023");
    assert(srv.getRepo().getEvents()[1].getDate() == "16/11/2022");
    assert(srv.getRepo().getEvents()[2].getDate() == "03/11/2022");

    std::vector<Event> events = srv.getRepo().getEvents();
    Service::sortRepoByDate(events);

    assert(events[0].getDate() == "03/11/2022");
    assert(events[1].getDate() == "16/11/2022");
    assert(events[2].getDate() == "16/10/2023");

    std::vector<Event> repo = srv.filterRepoByMonth(11);
    assert(repo.size() == 2);
    repo = srv.filterRepoByMonth(10);
    assert(repo.size() == 1);
    ...
    Event e2{};
    try {
        srv.deleteEventService("aaa", "12/12/2012");
    }
    catch (RepositoryException& rerr)
    {
        assert(rerr.what() == "Repository error: nonexistent event!");
    ...
    Event e3{ "first", "cluj-napoca", "16/11/2022", "19:00", 177, "https" };
    Event e4{ "second", "cluj-napoca", "03/11/2022", "19:00", 177, "https" };
    srv.addEventService("first", "cluj-napoca", "16/11/2022", "19:00", 177, "https");
    srv.addEventToUserList(e3);
    try {
        srv.addEventToUserList(e3);
    }
    catch (RepositoryException& rerr)
    ...
    srv.addEventService("second", "cluj-napoca", "03/11/2022", "19:00", 177, "https");
    srv.addEventToUserList(e4);

    srv.deleteEventUserService("first", "16/11/2022");
    try {
        srv.deleteEventUserService("second", "10/10/2019");
    }
    catch (RepositoryException& rerr)
    {
        assert(rerr.what() == "Repository error: nonexistent event!");
...
void Tests::test_validation()
{
    ...
    try
    {
        Validator::validateEventNrOfPeople("");
    }
    catch(ValidatorException& e)
    {
    }
    ...
    try
    {
        Validator::validateEventNrOfPeople("aaa");
    }
    catch(ValidatorException& e)
    {
    }
